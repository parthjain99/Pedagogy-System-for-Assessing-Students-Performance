...
if ($result->num_rows > 0) {
	...
	$sph = $row['phone'];
	if ($sem == $email) {
	 header("location:../admin.php?rp=1189");	
	}else{
	
	if ($sph == $phone) {
	 header("location:../admin.php?rp=2074");	
	}else{
...
VALUES ('$teacher_id', '$fname', '$lname', '$gender', '$dob', '$address', '$email', '$phone', '$department', '$category','admin')";

if ($conn->query($sql) === TRUE) {
  header("location:../admin.php?rp=5114");
} else {
  header("location:../admin.php?rp=5115");
}
